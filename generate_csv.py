...
def assign_values(filename, idx, list_to_assign):
    ...
    # read img to get width and height
    img = cv2.imread('C:/tensorflow1/models/research/object_detection/kitti_image/train/'+filename)
    sp = img.shape
    height = sp[0]
    width = sp[1]
    ...
    # bbox coordinates
    df.at[idx, 'xmin'] = str(int(float(list_to_assign[4])))
    df.at[idx, 'ymin'] = str(int(float(list_to_assign[5])))
    df.at[idx, 'xmax'] = str(int(float(list_to_assign[6])))
    df.at[idx, 'ymax'] = str(int(float(list_to_assign[7])))

...
def getContours(img, cThr=[100, 100], showCanny=False, minArea=1000, filter=0, draw=False):
    ...
    cv2.imshow("dial", cv2.resize(imgDial, (200, 200)))
    imgThre = cv2.erode(imgDial, kernel, iterations=2)  # 腐蚀运算
    cv2.imshow("erode", cv2.resize(imgThre, (200, 200)))
    if showCanny:
    ...
    if draw:
        for con in finalCountours:
            ...
            cv2.drawContours(img, con[4], cv2.FILLED, (0, 0, 255), 60)
...
'''
def warpImg(img, points, w, h, pad=80):
    # print(points)
    # 按照上下左右重新排序四个点
...
def findLine(img, show_edges=False, show_results=False, hough_thresh = 118):
    ...
    try:
        ...
        cv2.putText(result, 'Angle:' + str(int(avg_angel)), (5, 30), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.5,
                    (0, 0, 255), 2)
    except:
